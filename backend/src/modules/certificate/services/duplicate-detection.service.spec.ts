...
describe('DuplicateDetectionService', () => {
  ...
  describe('checkForDuplicates', () => {
    ...
    it('should respect time windows in rules', async () => {
      ...
      // The service constrains the query with `certificate.issuedAt >= cutoffDate`
      // for rules with a timeWindow. The mock repository simulates that the old
      // certificate (issued outside the 30-day window) is excluded by the query.
      const queryBuilder = {
        where: jest.fn().mockReturnThis(),
        andWhere: jest.fn().mockReturnThis(),
